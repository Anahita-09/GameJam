// Arrow keys to move and collect letters
let player, ground, platforms, coins, monsters;
let score = 0;
let gameState = "home";
let levelButtons = [];
let grassBg, iceBg, lavaBg, sandBg;
let blockImg, guyImg, letterImg, monsterImg;
...
function setup() {
  ...
  player.overlaps(coins, collect); // grabs letters
...
function draw() {
  if (gameState === "home") {
    ...
    textAlign(LEFT);
    textSize(28);
    text("Letters = " + score, 20, 60);
    textSize(20);
    text("Collect all 10 letters to win!", 20, 30);
